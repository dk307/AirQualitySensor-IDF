...
#define CRLF_LEN (sizeof(CRLF_STR) - 1)
  ...
  static const char *const TAG = "async_web_server";
  ...
  {
    return this->get_header("Host").value();
  }
  ...
  static std::string url_decode(const std::string &in)
  {
    ...
    for (std::size_t i = 0; i < in.size(); ++i)
    {
      if (in[i] == '%')
      {
        ...
        if (i + 1 < in.size())
        {
          auto c = esp32::parse_hex<uint8_t>(&in[i], 2);
          if (c.has_value())
          {
  ...
  void AsyncResponse::addHeader(const char *name, const char *value)
  {
  ...
  {
    this->print(std::to_string(value));
  }
  ...
  void AsyncEventSourceResponse::send(const char *message, const char *event, uint32_t id, uint32_t reconnect)
  {
    ...
    if (reconnect)
    {
      ...
      ev.append(std::to_string(reconnect));
    ...
    auto cs = esp32::str_snprintf("%x" CRLF_STR, 4 * sizeof(ev.size()) + CRLF_LEN, ev.size());
